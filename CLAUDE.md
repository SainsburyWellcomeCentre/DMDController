...
│   │                                            displaySequence, on, off, halt, clear,
│   │                                            setFrameRate, setInversion, setUpsideDown,
│   │                                            setLeftRightFlip, getTemperatures, getInfo,
│   │                                            allocSequence, startContinuous, startFinite,
│   │                                            waitForCompletion
│   │
...
│   │                                   Properties: driver, deviceId, width, height
...
│   │                                            setInversion, setUpsideDown, setLeftRightFlip,
│   │                                            setDMDMode, getTemperatures, getInfo,
│   │                                            getAllSequenceIds, allocSequence
│   │
│   ├── Sequence.m                   ← Manages one sequence (replaces @alpsequence)
│   │                                   Properties: driver, deviceId, sequenceId, bitPlanes,
│   │                                               picNum, width, height
│   │                                   Methods: alloc, free, control, timing, inquire,
│   │                                            put, setRepeat, setBitDepth, setBinaryMode,
│   │                                            setAOI, setTimingFromFPS
│   │
│   └── private/
...
│
└── examples/
    ├── basic_display.m              ← connect → 7 test patterns (incl. 8-bit) → disconnect
    ├── sequence_display.m           ← multi-frame sequence with timing
    ├── trigger_example.m            ← slave/trigger mode demo
    ├── clear_dmd_memory.m           ← demo of dmd.clear() and memory reclamation
    ├── get_dmd_info.m               ← query device info, list active sequence IDs
    ├── memory_vs_onthefly.m         ← benchmark: on-the-fly vs pre-loaded sequence
...
- Temperature inquiry: `getTemperatures()` → returns struct with `ddc_fpga`, `apps_fpga`, `pcb` (deg C, 1/256 resolution); warns if `apps_fpga` exceeds device max ✅
- `getInfo()` → returns struct with `serialNumber`, `version`, `availMemory`, `width`, `height` ✅
- `getAllSequenceIds()` → scans IDs 0–127 via `AlpSeqInquire(ALP_BITPLANES)`; returns array of currently valid sequence IDs ✅
- `allocSequence(bitPlanes, picNum)` → constructs and returns a `Sequence` object ✅
...
- `put(picOffset, picLoad, imageData)` — transfers image data to device SDRAM; auto-scales/pads any image size to fit `[height x width]` via `p_padCrop` (nearest-neighbour, aspect-ratio-preserving, centered); accepts `uint8`, `logical`, or numeric ✅
- `setRepeat(nRepeat)` — sets `ALP_SEQ_REPEAT` (0 = continuous) ✅
- `setBitDepth(bits)` — reduces displayed bit depth via `ALP_BITNUM` ✅
- `setBinaryMode(uninterrupted)` — `ALP_BIN_NORMAL` or `ALP_BIN_UNINTERRUPTED` (reduces flicker for 1-bit sequences) ✅
- `setAOI(startRow, rowCount)` — restricts display to a row range via `ALP_SEQ_DMD_LINES` ✅
- `setTimingFromFPS(fps)` — configures `illuminateTime`/`pictureTime`; queries `ALP_MIN_ILLUMINATE_TIME`; uses 100% duty cycle for 1-bit, 95% for multi-bit ✅
- `timing(illuminateTime, pictureTime, synchDelay, synchPulseWidth, triggerInDelay)` — direct `AlpSeqTiming` wrapper (all in µs) ✅
- `control(controlType, controlValue)` — direct `AlpSeqControl` wrapper ✅
- `inquire(inquireType)` — direct `AlpSeqInquire` wrapper ✅
- `putLines(picOffset, picLoad, lineOffset, lineLoad, imageData)` — wraps `AlpSeqPutEx` with `tAlpLinePut` *(future)*
- `setFLUT(mode, entries, frameNumbers)` — wraps FLUT constants + `AlpProjControlEx(ALP_FLUT_WRITE_9BIT, tFlutWrite)` *(future)*
- `setBitplaneLUT(mode, entries)` — wraps `ALP_BITPLANE_LUT_MODE` *(future)*
- `setScrolling(firstLine, lastLine, lineInc)` — wraps scrolling constants *(future)*
- `setDynSynch(period, pulseWidth)` — wraps `ALP_SEQ_DYN_SYNCH_OUT_PERIOD/PULSEWIDTH` *(future)*
...
dmd = DMDController.DMD();        % create (does not connect yet)
dmd.connect();                    % or: dmd.connect(deviceNum)
dmd.displayFrame(myImage);        % show a [H x W] uint8/logical frame continuously
dmd.displayFrame(myImage, fps);   % with explicit frame rate
dmd.displayFrame(myImage, fps, bitDepth); % bitDepth 1–8 (default 1); use 8 for grayscale
dmd.displayFrame('path/to/img'); % accepts filename string; RGB is auto-converted
dmd.displaySequence(imageStack, fps, nRepeat);   % multi-frame; nRepeat=0 → infinite
dmd.displaySequence(imageStack, fps, nRepeat, bitDepth);
dmd.setFrameRate(fps);            % change rate of current sequence
dmd.waitForCompletion();          % block until finite sequence finishes
dmd.on();  dmd.off();             % all-white / all-black convenience methods
dmd.halt();                       % stop projection (keeps sequence allocated)
dmd.clear();                      % halt + free current sequence memory
dmd.setInversion(tf);             % reverse dark/bright
dmd.setUpsideDown(tf);            % flip image upside down
dmd.setLeftRightFlip(tf);         % flip image left/right
dmd.getTemperatures();            % struct: ddc_fpga, apps_fpga, pcb (deg C)
dmd.getInfo();                    % struct: serialNumber, version, availMemory, width, height
seq = dmd.allocSequence(bits, n); % allocate custom Sequence; caller responsible for delete()
dmd.startContinuous(seq);         % project custom sequence continuously
dmd.startFinite(seq, nRepeat);    % project custom sequence N times
dmd.disconnect();                 % halt + free all sequences + free device
```

**`clear()` vs `halt()` vs `disconnect()`:**
| Method | Stops projection | Frees `p_seq` memory | Frees device |
|--------|-----------------|----------------------|--------------|
| `halt()` | ✅ | ✗ | ✗ |
| `clear()` | ✅ | ✅ | ✗ |
| `disconnect()` | ✅ | ✅ | ✅ |

Internally `DMD` constructs `Driver`, `Device`, and `Sequence` objects and orchestrates them. `dmd.device` and `dmd.driver` are exposed as read-only properties for advanced users who need direct access.
...
- [x] **`DMD.m`** — Facade with `connect`, `disconnect`, `displayFrame` (filename/RGB/bitDepth), `displaySequence`, `waitForCompletion`, `on`, `off`, `halt`, `clear`, `setFrameRate`, `setInversion`, `setUpsideDown`, `setLeftRightFlip`, `getTemperatures`, `getInfo`, `allocSequence`, `startContinuous`, `startFinite`.
- [x] **`Device.m`** additions — `getAllSequenceIds()` (scans IDs 0–127).
- [x] **`Sequence.m`** additions — `p_padCrop` private helper: nearest-neighbour scaling to fit any image into the DMD canvas (aspect-ratio-preserving, centered, zero-padded).
- [x] **`examples/`** — `basic_display.m` (7 test patterns incl. 8-bit grayscale), `sequence_display.m`, `trigger_example.m`, `clear_dmd_memory.m`, `get_dmd_info.m`, `memory_vs_onthefly.m`, `upload_with_check.m`.
...
- [x] `Sequence.p_padCrop()` — auto-scales any input image to the DMD canvas (nearest-neighbour, aspect-ratio-preserving, centered)
